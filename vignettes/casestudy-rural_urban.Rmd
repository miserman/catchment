...
Well also use a map to show comparisons between the focal regions:
```{r}
...
tracts <- tracts[grep("^51(?:107|059)", tracts$geoid), ]
...
block_groups <- block_groups[grep("^51(?:107|059)", block_groups$geoid), ]
...
get_focal_subset <- function(data, rowIds, counties = c(Loudoun = "51107", Fairfax = "51059")) {
  comp <- colnames(data)[1:2]
  sub <- do.call(rbind, unname(lapply(counties, function(county) {
    d <- data[startsWith(rownames(data), county), ]
    n <- nrow(d)
