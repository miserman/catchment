...
relates to how well they fall into the catchment area; someone on the edge of a catchment area is not as well caught
as someone nearer its center. To appreciate this difference, we can adjust weights by distance.

In the previous example, imagine consumers 1 and 2 are both close to the first provider, but on opposite sides,
...
This may be counterintuitive when relative difference are held constant. For example, if a catchment area includes
...
they are the only location in range -- otherwise scores will infinity approach 0 and 1 as relative difference in
weights increase.
...
splot(
  ...
  t(vapply(seq(1, 100, .5), function(i) catchment_ratio(
    consumers, providers, weight = matrix(c(.9, .9 * i, 0, 0, .2 * i, .9), 3)
...
catchment_ratio(
  consumers, providers, weight = matrix(c(.9, .9 * 1, 0, 0, .2 * 1, .9), 3)
)

# to having nearly exclusive access
catchment_ratio(
  consumers, providers, weight = matrix(c(.9, .9 * 100, 0, 0, .2 * 100, .9), 3)
)

# but they will never have entirely exclusive access so long as consumers 1 and 3 have any weight
catchment_ratio(
  consumers, providers, weight = matrix(c(.9, .9 * 1e99, 0, 0, .2 * 1e99, .9), 3)
)
```
...
splot(
  t(vapply(seq(1, 100, .5), function(i) catchment_ratio(
    consumers, providers, weight = matrix(c(.9, .9 * i, 0, 0, .2 * i, .9), 3) / c(.9, 1.1 * i, .9)
...
(access_colnorm <- catchment_ratio(
  consumers, providers, weight = sweep(weight, 2, colSums(weight), "/")
))
...
# then remove them, and add those who do commute to their home-origin as well
diag(consumers_work) <- 0
diag(consumers_work) <- rowSums(consumers_work)

# calculate proportional distribution of commuters
(consumers_work <- consumers_work / rowSums(consumers_work))

# use this and proportional weights to calculate commuter weights
(weight_commute <- consumers_work %*% weight_prop * rowSums(weight))
...
### Multiple Modes
We have also been assuming consumes all use the same mode of travel, such that their travel
...
consumer location did not have access to a car, and would walk to their provider. We can combined these in the
