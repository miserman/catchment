...
distributed across all regions in range, and in this case, that distribution is uniform over
distances.
...
doctors (in blue), though this seems to mostly happen in Virginia regions, and nearly
reverse in Maryland regions.
...
the larger regions to the north and south of the main metropolitan area (which have mostly
negative values, showing they get lower scores with continuous weights).
...
Differences are clearest where regions fall between clusters of doctors, but are not very
...
map |> addControl(
    paste("Difference Between 3-Step Floating Catchment Areas with Single (lower) VS",
    "Multiple (higher) origins"), "topright"
  ) |>
  addProviderTiles("CartoDB.Positron") |>
